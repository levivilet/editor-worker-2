...
export const name = 'editor.matrix-235-clipboard-copy'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-235.txt`
  await FileSystem.writeFile(filePath, 'copy source 9')
  ...
  await expect(line0).toHaveText('copy source 9copy')
