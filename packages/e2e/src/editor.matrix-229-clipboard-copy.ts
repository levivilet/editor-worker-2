...
export const name = 'editor.matrix-229-clipboard-copy'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-229.txt`
  await FileSystem.writeFile(filePath, 'copy source 3')
  ...
  await expect(line0).toHaveText('copy source 3copy')
