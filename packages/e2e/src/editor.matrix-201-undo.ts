...
export const name = 'editor.matrix-201-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-201.txt`
  await FileSystem.writeFile(filePath, 'undo-0')
  ...
  await expect(line0).toHaveText('undo-0')
