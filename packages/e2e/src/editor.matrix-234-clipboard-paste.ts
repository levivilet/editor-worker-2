...
export const name = 'editor.matrix-234-clipboard-paste'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-234.txt`
  await FileSystem.writeFile(filePath, 'clip-8')
  await Workspace.setPath(tmpDir)
  await Main.openUri(filePath)
  await ClipBoard.enableMemoryClipBoard()
  await ClipBoard.writeText('paste-8')
  ...
  await expect(line0).toHaveText('clip-8paste-8')
