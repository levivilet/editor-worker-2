...
export const name = 'editor.matrix-236-clipboard-cut'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-236.txt`
  await FileSystem.writeFile(filePath, 'cut source 10')
  ...
  await expect(line0).toHaveText('cut source 10')
