...
export const name = 'editor.matrix-241-clipboard-copy'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-241.txt`
  await FileSystem.writeFile(filePath, 'copy source 15')
  ...
  await expect(line0).toHaveText('copy source 15copy')
