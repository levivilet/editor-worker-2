...
export const name = 'editor.matrix-239-clipboard-cut'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-239.txt`
  await FileSystem.writeFile(filePath, 'cut source 13')
  ...
  await expect(line0).toHaveText('cut source 13')
