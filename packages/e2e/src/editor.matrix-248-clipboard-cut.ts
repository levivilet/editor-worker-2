...
export const name = 'editor.matrix-248-clipboard-cut'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-248.txt`
  await FileSystem.writeFile(filePath, 'cut source 22')
  ...
  await expect(line0).toHaveText('cut source 22')
