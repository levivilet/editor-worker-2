...
export const name = 'editor.matrix-206-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-206.txt`
  await FileSystem.writeFile(filePath, 'undo-5')
  ...
  await expect(line0).toHaveText('undo-5')
