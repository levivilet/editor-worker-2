...
export const name = 'editor.matrix-247-clipboard-copy'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-247.txt`
  await FileSystem.writeFile(filePath, 'copy source 21')
  ...
  await expect(line0).toHaveText('copy source 21copy')
