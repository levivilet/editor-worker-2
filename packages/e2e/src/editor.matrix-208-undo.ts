...
export const name = 'editor.matrix-208-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-208.txt`
  await FileSystem.writeFile(filePath, 'undo-7')
  ...
  await expect(line0).toHaveText('undo-7')
