...
export const name = 'editor.matrix-217-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-217.txt`
  await FileSystem.writeFile(filePath, 'redo-1')
  ...
  await expect(line0).toHaveText('redo-1!')
