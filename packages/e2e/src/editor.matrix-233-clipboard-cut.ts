...
export const name = 'editor.matrix-233-clipboard-cut'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-233.txt`
  await FileSystem.writeFile(filePath, 'cut source 7')
  ...
  await expect(line0).toHaveText('cut source 7')
