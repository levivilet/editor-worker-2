...
export const name = 'editor.matrix-222-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-222.txt`
  await FileSystem.writeFile(filePath, 'redo-6')
  ...
  await expect(line0).toHaveText('redo-6!')
