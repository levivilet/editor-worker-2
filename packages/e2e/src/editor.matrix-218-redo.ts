...
export const name = 'editor.matrix-218-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-218.txt`
  await FileSystem.writeFile(filePath, 'redo-2')
  ...
  await expect(line0).toHaveText('redo-2!')
