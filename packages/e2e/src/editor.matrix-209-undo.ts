...
export const name = 'editor.matrix-209-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-209.txt`
  await FileSystem.writeFile(filePath, 'undo-8')
  ...
  await expect(line0).toHaveText('undo-8')
