...
export const name = 'editor.matrix-251-load-crlf'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-251.txt`
  await FileSystem.writeFile(filePath, 'load-0\r\nsecond-0\r\nthird-0')
  ...
  await expect(line0).toHaveText('load-0')
  const line1 = lines.nth(1)
  await expect(line1).toHaveText('second-0')
  const line2 = lines.nth(2)
  await expect(line2).toHaveText('third-0')
