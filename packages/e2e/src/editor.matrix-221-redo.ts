...
export const name = 'editor.matrix-221-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-221.txt`
  await FileSystem.writeFile(filePath, 'redo-5')
  ...
  await expect(line0).toHaveText('redo-5!')
