...
export const name = 'editor.matrix-204-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-204.txt`
  await FileSystem.writeFile(filePath, 'undo-3')
  ...
  await expect(line0).toHaveText('undo-3')
