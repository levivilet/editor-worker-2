...
export const name = 'editor.matrix-210-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-210.txt`
  await FileSystem.writeFile(filePath, 'undo-9')
  ...
  await expect(line0).toHaveText('undo-9')
