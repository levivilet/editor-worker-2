...
export const name = 'editor.matrix-246-clipboard-paste'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-246.txt`
  await FileSystem.writeFile(filePath, 'clip-20')
  await Workspace.setPath(tmpDir)
  await Main.openUri(filePath)
  await ClipBoard.enableMemoryClipBoard()
  await ClipBoard.writeText('p20\nq20')
  ...
  await expect(line0).toHaveText('clip-20p20')
  const line1 = lines.nth(1)
  await expect(line1).toHaveText('q20')
