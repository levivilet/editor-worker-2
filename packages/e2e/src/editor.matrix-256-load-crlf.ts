...
export const name = 'editor.matrix-256-load-crlf'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-256.txt`
  await FileSystem.writeFile(filePath, 'load-5\r\nsecond-5\r\nthird-5')
  ...
  await expect(line0).toHaveText('load-5')
  const line1 = lines.nth(1)
  await expect(line1).toHaveText('second-5')
  const line2 = lines.nth(2)
  await expect(line2).toHaveText('third-5')
