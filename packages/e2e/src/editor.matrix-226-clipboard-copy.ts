...
export const name = 'editor.matrix-226-clipboard-copy'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-226.txt`
  await FileSystem.writeFile(filePath, 'copy source 0')
  ...
  await expect(line0).toHaveText('copy source 0copy')
