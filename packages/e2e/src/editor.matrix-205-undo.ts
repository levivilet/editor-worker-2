...
export const name = 'editor.matrix-205-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-205.txt`
  await FileSystem.writeFile(filePath, 'undo-4')
  ...
  await expect(line0).toHaveText('undo-4')
