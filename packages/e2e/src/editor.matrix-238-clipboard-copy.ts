...
export const name = 'editor.matrix-238-clipboard-copy'

export const test: Test = async ({ ClipBoard, Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-238.txt`
  await FileSystem.writeFile(filePath, 'copy source 12')
  ...
  await expect(line0).toHaveText('copy source 12copy')
