...
export const name = 'editor.matrix-202-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-202.txt`
  await FileSystem.writeFile(filePath, 'undo-1')
  ...
  await expect(line0).toHaveText('undo-1')
