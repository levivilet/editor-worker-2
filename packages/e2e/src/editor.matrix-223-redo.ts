...
export const name = 'editor.matrix-223-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-223.txt`
  await FileSystem.writeFile(filePath, 'redo-7')
  ...
  await expect(line0).toHaveText('redo-7!')
