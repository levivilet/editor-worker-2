...
export const name = 'editor.matrix-257-load-crlf'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-257.txt`
  await FileSystem.writeFile(filePath, 'load-6\r\nsecond-6\r\nthird-6')
  ...
  await expect(line0).toHaveText('load-6')
  const line1 = lines.nth(1)
  await expect(line1).toHaveText('second-6')
  const line2 = lines.nth(2)
  await expect(line2).toHaveText('third-6')
