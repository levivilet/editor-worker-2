...
export const name = 'editor.matrix-224-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-224.txt`
  await FileSystem.writeFile(filePath, 'redo-8')
  ...
  await expect(line0).toHaveText('redo-8!')
