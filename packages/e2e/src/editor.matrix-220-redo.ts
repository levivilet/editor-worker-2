...
export const name = 'editor.matrix-220-redo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-220.txt`
  await FileSystem.writeFile(filePath, 'redo-4')
  ...
  await expect(line0).toHaveText('redo-4!')
