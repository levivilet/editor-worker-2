...
export const name = 'editor.matrix-207-undo'

export const test: Test = async ({ Command, expect, FileSystem, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/matrix-207.txt`
  await FileSystem.writeFile(filePath, 'undo-6')
  ...
  await expect(line0).toHaveText('undo-6')
